...
def get_resutl(quest,ans_a,ans_b,ans_c,ans_or):
    def get_rate_a():
        ...
        global nav
        if 'KHÔNG' in quest:
            rate_a = analys_notdetach.negative_question(quest, ans_a)
            nav = 1
        elif '"' in quest:
            rate_a = analys_notdetach.quotation_mark(quest, ans_a)
        else:
            print('======================================================================')
            rate_a = analys_notdetach.normal_question(quest, ans_a)
    def get_rate_b():
        global rate_b
        global nav
        if 'KHÔNG' in quest:
            rate_b = analys_notdetach.negative_question(quest, ans_b)
        elif '"' in quest:
            rate_b = analys_notdetach.quotation_mark(quest,ans_b)
        else:
            rate_b = analys_notdetach.normal_question(quest, ans_b)
    def get_rate_c():
        global rate_c
        global nav
        if 'KHÔNG' in quest:
            rate_c = analys_notdetach.negative_question(quest, ans_c)
        elif '"' in quest:
            rate_c = analys_notdetach.quotation_mark(quest,ans_c)
            # if rate_c[ans_c] == 0:
            #     rate_c = detach_answer.quotation_mark_ans(quest, ans_c)
        else:
            rate_c = analys_notdetach.normal_question(quest, ans_c)
